...
| GUI self-update | GitHub release API + binary ZIP/checksums | network + installation filesystem | **In — downloaded code replaces the trusted distribution** *(BreakTest-specific)* |
...
- **The release service→installation path IS a trust boundary.** GUI updates fetch stable-release metadata and assets over HTTPS from the Breaking-IT GitHub repository. The binary is staged outside the installation and checked against GitHub's SHA-256 asset digest and the published SHA-512 file before an external helper replaces files after the GUI exits. Archive extraction and installation paths are confined, source checkouts are refused, and replacement is rolled back on failure. These hashes detect corruption or mismatched assets but are hosted by the same release platform; they are not an independent publisher signature. *(BreakTest-specific)*
...
- **Negative side-effects inventory:** JMeter makes outbound network requests to the SUT by design; in recording mode it listens on a local proxy port and writes a CA keystore; it reads/writes test plans, results, and properties on the local filesystem; it executes user-supplied scripting. BreakTest's GUI also makes rate-limited update checks and, only after explicit user action, replaces a writable binary installation and restarts. *(maintainer / documented / BreakTest-specific)*
...
| GitHub release metadata/assets | executable distribution update | yes — remote trust boundary | accept only expected stable binary assets over HTTPS; verify digests before installation |
...
- **Release-path attacker** — attempts to substitute an update archive, corrupt release metadata, exploit archive paths, or redirect installation writes outside the BreakTest home. In-model; mitigated by HTTPS, two digest checks, bounded extraction, path/symlink confinement, post-exit installation, and rollback. Compromise of the GitHub repository/release account remains a supply-chain risk because the current hashes are not an independent signature. *(BreakTest-specific)*
...
- **GUI updates are verified and installation-scoped.** BreakTest accepts only the expected stable binary ZIP from GitHub over HTTPS, verifies the GitHub SHA-256 digest and published SHA-512 checksum, rejects traversal and symlinked installation paths, refuses source checkouts, preserves user settings/plugins, and rolls back replaced files when installation fails. *(BreakTest-specific)*
...
- **The GUI updater does not yet verify an independent publisher signature.** The SHA-256 asset digest and SHA-512 file are both obtained from GitHub. They protect against corruption and mismatched downloads, but a compromised release account could replace the binary and both hashes. *(BreakTest-specific)*
